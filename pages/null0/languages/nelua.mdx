...
- Check out the [Lua template project](https://github.com/notnullgames/cart_nelua)
...
BUILD_CART_NELUA() {
  ...
  cd "/tmp/${name}"
  cp -R "${source_dir}"/* .
  nelua -L "${SCRIPT_DIR}" "${source_dir}/main.nelua" -r --cc "/opt/wasi-sdk/bin/clang" -o ./main.wasm
